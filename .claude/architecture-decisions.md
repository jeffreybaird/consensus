  ...
  class Create
    ...
    def call(account:, actor:, attrs:)
      ...
      if note.save                                                   # false + note.errors on invalid
        Consensus::Audit.record("note.created", resource: note, actor: actor)  # side effects: see §5
        Success(note)
      else
...
# Consensus::Result.err(:plan_limit_reached, { limit: 100, current: 100 })
...
Consensus::Audit.record("note.created", resource: note, actor: actor)
```
...
note.soft_delete
Consensus::Audit.record("note.deleted", resource: note)
```
...
def call(...)
  ...
  Consensus::Audit.record("note.created", resource: note, actor:)              # inline: cheap, durable (§2)
  Consensus::WebhookJob.perform_async(note.account_id, "note.created", note.to_hash)  # enqueued (Sidekiq)
  Consensus::Cache.delete("account:#{note.account_id}:note_count")             # bust the cache key (if Redis cache)
...
| Side effect | How |
|-------------|-----|
| Audit log | `Consensus::Audit.record(…)` inline after commit (§2) — cheap, must not be lost |
| Webhook dispatch | `Consensus::WebhookJob.perform_async(account_id, event, payload)` (Sidekiq) |
| Notification email | enqueue a Sidekiq job that sends via `Mail`/`Pony` — there is no ActionMailer |
| Cache invalidation | `Consensus::Cache.delete(…)` over Redis — only if a cache is configured (`app/cache.rb`) |
...
```ruby
Consensus::Flags.enabled?(:new_editor, actor: Current.user)   # per-actor / percentage
Consensus::Flags.enable_percentage(:new_editor, 25)           # gradual rollout
Consensus::Flags.enable_for_account(:beta_export, account)    # by account (group)
```

- ✅ `return Failure([:feature_not_enabled]) unless Consensus::Flags.enabled?(:export, account:)`
...
Consensus::WebhookJob.perform_async(account.id, event, payload)
```
