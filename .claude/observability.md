...
1. **Every business mutation gets a manual span.** If a service object creates,
   ...
   `consensus.<context>.<operation>`. No library knows your business operations.
...
OTEL_SERVICE_NAME=consensus          # overrides service_name above if set
```
...
tracer.in_span("consensus.billing.cancel_subscription") { ... }

# ❌ Manual span wrapping a plain Sequel query — -sequel already covers it (when active)
tracer.in_span("consensus.notes.get") { Note[id] }
```
...
module Notes
  class Create
    TRACER = OpenTelemetry.tracer_provider.tracer("consensus.notes")

    def self.call(account:, params:)
      TRACER.in_span("consensus.notes.create") do |span|
        span.set_attribute("consensus.account.id", account.id)
...
span.set_attribute("consensus.invoice.id", invoice.id)
...
```ruby
TRACER.in_span("consensus.payments.charge") do |span|
  charge!
...
```
consensus.<context>.<operation>
```

Examples:
- `consensus.notes.create`
- `consensus.notes.update`
- `consensus.billing.create_checkout`
- `consensus.billing.cancel_subscription`
- `consensus.accounts.invite_member`
- `consensus.webhooks.dispatch`
- `consensus.imports.process_record`

External services use the service name:
- `consensus.storage_provider.create_upload_url`
- `consensus.payment_provider.create_subscription`

Jobs (only if you add Sidekiq, and only for an enrichment span):
- `consensus.job.webhook_event_processor`
- `consensus.job.analytics_aggregation`

---

## Span Attributes

### Business attributes — namespace under `consensus.*`

```ruby
span.set_attribute("consensus.account.id", account.id)  # tenant/org scope
span.set_attribute("consensus.user.id", user.id)        # user-initiated ops
```
...
```ruby
span.add_attributes(
  "consensus.service" => "storage_provider",
  "consensus.idempotency_key" => key
)
```
...
```ruby
span.add_attributes(
  "consensus.account.id" => args["account_id"],
  "messaging.system" => "sidekiq",   # semconv, usually auto-set
  "consensus.job.attempt" => attempt
)
```
...
For your own business attributes, namespace under `consensus.*`. Do **not** re-emit
a semconv attribute under a custom name, and do not duplicate one the
auto-instrumentation already sets.

```ruby
# ✅ semconv for HTTP/DB (auto), consensus.* for business attrs
span.set_attribute("consensus.account.id", account.id)

# ❌ duplicating a semconv attribute under a custom name
span.set_attribute("consensus.http_status", 200)  # http.response.status_code already set
```
...
```ruby
# ✅ ID only
span.set_attribute("consensus.user.id", user.id)

# ❌ PII in the telemetry pipeline
span.set_attribute("consensus.user.email", user.email)
```
...
class WebhookEventJob
  ...
  def perform(account_id, event_id)
    OpenTelemetry::Trace.current_span.set_attribute("consensus.account.id", account_id)
...
class App < Sinatra::Base
  before do
    ...
    OpenTelemetry::Trace.current_span.add_attributes(
      "consensus.account.id" => Current.account&.id,
      "consensus.user.id"    => Current.user&.id
    )
  end
...
- [ ] Business mutations wrapped in a `consensus.<context>.<operation>` span
- [ ] Span attributes include `consensus.account.id` for tenant/user-scoped ops
- [ ] External API calls span with `consensus.service` + idempotency key
