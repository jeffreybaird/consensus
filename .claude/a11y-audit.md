...
Every UI addition or modification in `Consensus` MUST fully comply with WCAG 2.1 AA. This is
...
- Every `<img>` MUST have a meaningful `alt` (or `alt=""` if purely decorative).
  - **Sinatra/ERB:** `<img src="/img/logo.png" alt="Consensus">` — never omit `alt`.
